...
This REP seeks to standardize an interface for IMU drivers in the ROS ecosystem, providing for IMU-type sensors with a variety of capabilities and conventions. It formalizes a common approach based on existing driver implementations as a guideline for new driver development, and minimizes the changes required to non-conforming implementations. This REP also specifies the limits wherein sensor data will be properly handled by ROS transform and coordinate specifications, and discourages the in-driver manipulation of sensor data, unless that data will break ROS specifications. It then provides fall-back manipulation approaches and guidelines for keeping the consistency of sensor data within user expectation and manufacturer specification.
