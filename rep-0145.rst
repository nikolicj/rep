...
* The device **body frame** represents the device's internal reference frame (the "input reference axes", IRA). It may be found in the device specification documents, and is sometimes indicated directly on the package of the device. This frame is fixed to the device (i.e. it is a moving coordinate frame).

* If an orientation estimate is provided (see `Data Reporting`_), the device **world frame** represents the external reference with respect to which the **body frame** is indicated. Two out of three degrees of freedom of the **body frame** w.r.t. the **world frame** are usually considered "fixed" through gravity, with the relevant conventions from REP 103 [1]_ :
...
* Accelerometers
  ...
  - The accelerometers report linear acceleration data expressed in the body frame of the device. This data is output from the driver as a 3D vector, representing the specific force acting on the sensor.
...
* Gyroscopes
  ...
  - The gyroscopes report the rotational velocity of the body frame w.r.t. an inertial frame, expressed in the body frame. This data is provided from the driver as a 3D vector.
...
* Orientation
  ...
  - The IMU sensor may provide a fused orientation estimate. This data is output from the driver in the form of a quaternion, which represents the orientation of the body frame w.r.t. the world frame.
