...
* If an orientation estimate is provided (see `Data Reporting`_), the **world frame** represents the external reference frame for the device. This frame is fixed in one of two orientations, with the relevant conventions from REP 103 [1]_ :
  ...
  - For NED type IMUs, the orientation is x-forward, y-right, and z-down. If an absolute yaw reference relative to magnetic north is available via the magnetometer, the orientation is x-north, y-east, z-down.

  - For ENU type IMUs, the orientation is x-forward, y-left, and z-up. If an absolute yaw reference relative to magnetic north is available via the magnetometer, the orientation is x-east, y-north, z-up.
